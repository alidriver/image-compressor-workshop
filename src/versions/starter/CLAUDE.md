...
- Entry point: `App.tsx` (default-exported component, rendered by the root router when the URL hash is `#/starter`).
- Metadata: `meta.ts` exports `{ title, description }` consumed by the homepage card.
- Slug: derived from this folder's name (`starter`). Renaming the folder changes the URL.
...
There is no git in this project (intentionally). To preserve a working state before a risky pivot, **duplicate this entire folder** to a new slug (e.g. `starter-v2`). Both will appear as cards on the homepage so the user can compare them side-by-side.
